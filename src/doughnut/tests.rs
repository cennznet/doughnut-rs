...
fn it_works_v0() {
    ...
    assert_eq!(d.get_topping("something"), Some(&[0_u8][..]));
    assert_eq!(d.get_topping("somethingElse"), Some(&[0_u8][..]));
    assert_eq!(&d.signature[..], &payload[(payload.len() - 64) as usize..],);
    assert_eq!(d.encode(), payload);
}

#[test]
fn v0_encode_two_toppings() {
    let payload: Vec<u8> = vec![
        // version and topping count
...
fn versioned_enum_v0_sr25519_verification() {
    ...
    let encoded: Vec<u8> = vec![
        0x00, 0x00, 0x03, // version and topping count
